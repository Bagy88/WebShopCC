...
namespace WebShopCC.Controllers
{
    public class CartsController : ApiController
    {
        ...
        // GET: api/Carts
        public IQueryable<CartDdlDTO> GetCart()
        {
            var cart = db.Cart
                .Include(p => p.Product)
                .Select(c => new CartDdlDTO()
                {
                    Name = c.Product.Name,
        ...
        public async Task<IHttpActionResult> GetCart(string sessionid)
        {
            Cart cart = await db.Cart.Where(c => c.SessionID == sessionid).SingleOrDefaultAsync();
            if (cart == null)
            {
                return NotFound();
            }
